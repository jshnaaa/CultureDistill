...
class CulturePRM_v3(nn.Module):
    ...
    def score_batch(
        ...
        # Find step positions for each input
        all_step_positions = []
        valid_indices = []  # indices of inputs that have steps
        ...
        # If no valid inputs, return neutral scores
        if not valid_indices:
            return [0.5] * len(input_texts)

        # Batch tokenize only valid inputs
        ...
        # Score each input's step positions
        results = [0.5] * len(input_texts)  # default neutral
        seq_len = hidden_states.size(1)
...
def train(args):
    ...
    if not args.no_prm:
        ...
        prm = None
        prm_tokenizer = None
        print("  [PRM] Skipped (--no_prm mode, using R_outcome only)")
    ...
    print(f"Batches per round: {effective_batches_info} (prompt_batch={args.prompt_batch})")
    print(f"PRM scoring: {'DISABLED (--no_prm)' if args.no_prm else 'ENABLED (batch mode)'}")
    ...
    for rnd in range(1, args.max_rounds + 1):
        ...
        for batch_idx, batch in enumerate(loader):
            ...
            # Compute R_outcome for all samples
            r_outcomes = []
            ...
            # Compute R_process via batch PRM scoring (or skip if --no_prm)
            if args.no_prm:
                r_processes = [0.0] * (n_prompts * args.n_samples)
            else:
                # Build all PRM inputs at once
                prm_inputs = []
                ...
                # Batch score (process in sub-batches of 20 to limit GPU memory)
                PRM_BATCH_SIZE = 20
                r_processes = []
            ...
            # Phase A: Reference log-probs (eval mode, no grad, no gradient checkpointing)
            policy.eval()
...
def main():
    ...
    parser.add_argument("--n_samples", type=int, default=5,
                        help="Responses per prompt per round (default: 5)")
    ...
    parser.add_argument("--prompt_batch", type=int, default=8,
                        help="Prompts per optimizer step (default: 8)")
    ...
    parser.add_argument("--batches_per_round", type=int, default=130,
                        help="Max batches per round (sub-sampling). "
                             "Set to -1 to use all batches. (default: 130)")
