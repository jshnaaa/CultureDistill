        ...
        if guardian_response:
            ...
            if self.task_type == "cultureatlas":
                ...
                user = (
                    ...
                    f"As a Cross-Cultural Auditor from [{agent_name}] background:\n"
                    f"1. The Guardian has PRIMARY AUTHORITY on {target_country}'s culture. "
                    f"Give their factual claims strong weight.\n"
                    f"2. However, CHECK the question structure: if the question asks for "
                    f"something \"unusual\", \"NOT\", \"never\", or \"least\" common, verify "
                    f"the Guardian selected the OUTLIER (the one that does NOT fit).\n"
                    f"3. If the Guardian's answer is consistent with what the question asks, "
                    f"AGREE with the Guardian and explain why from your perspective.\n"
                    f"4. Only disagree if you detect a clear logical error (e.g., Guardian "
                    f"selected a common practice when the question asked for an unusual one).\n\n"
                    f"Answer format: first line is ONLY the number (1/2/3/4), "
                    f"second line is a brief explanation."
                )
            else:
    ...
    def _build_judge_prompt(self, question: str, target_country: str,
        ...
        if self.task_type == "cultureatlas":
            ...
            user = (
                ...
                f"STEP 2 - EVALUATE AGENT RESPONSES:\n"
                f"The HOST-CULTURE GUARDIAN is [{guardian_name}].\n"
                f"Agent responses:\n{responses_text}\n"
                f"DECISION RULES (apply in strict order):\n"
                f"1. The Guardian has PRIMARY AUTHORITY on cultural facts about "
                f"{target_country}. DEFAULT to the Guardian's answer unless there is "
                f"a clear reason not to.\n"
                f"2. ONLY override the Guardian if:\n"
                f"   (a) Guardian clearly misunderstood the question structure (e.g., "
                f"selected a COMMON practice when the question asked for something "
                f"UNUSUAL or NOT done), OR\n"
                f"   (b) Guardian's stated reasoning explicitly CONTRADICTS their own "
                f"answer choice.\n"
                f"3. Auditor disagreement alone is NOT sufficient to override the Guardian. "
                f"Auditors are less expert on {target_country}'s culture.\n"
                f"4. All option positions (1/2/3/4) are equally likely to be correct.\n\n"
                f"Answer format: first line is ONLY the number (1/2/3/4), "
