        ...
        if guardian_response:
            ...
            if self.task_type == "cultureatlas":
                ...
                user = (
                    f"TARGET CULTURE: {target_country}\n\n"
                    f"{question}\n\n"
                    f"The HOST-CULTURE GUARDIAN [{guardian_name}] has provided their answer:\n"
                    f"---\n{guardian_response.strip()}\n---\n\n"
                    f"As a Cross-Cultural Auditor from [{agent_name}] background:\n"
                    f"FIRST: Independently parse the question. If it contains negations "
                    f"(\"unusual\", \"NOT\", \"never\"), the correct answer should be the OUTLIER.\n"
                    f"THEN: Form your OWN answer. If your reasoning leads to a different "
                    f"answer than the Guardian, TRUST YOUR OWN ANALYSIS — the Guardian "
                    f"may have misread the question or made a factual error.\n"
                    f"Only agree with the Guardian if your independent analysis supports it.\n\n"
                    f"Answer format: first line is ONLY the number (1/2/3/4), "
                    f"second line is a brief explanation."
                )
            else:
    ...
    def _build_judge_prompt(self, question: str, target_country: str,
        ...
        if self.task_type == "cultureatlas":
            ...
            user = (
                ...
                f"STEP 2 - YOUR INDEPENDENT ANSWER:\n"
                f"Based on the question alone, what do you think the answer is?\n\n"
                f"STEP 3 - COMPARE WITH AGENTS:\n"
                f"The HOST-CULTURE GUARDIAN is [{guardian_name}].\n"
                f"Agent responses:\n{responses_text}\n"
                f"DECISION RULES:\n"
                f"- If Guardian AGREES with your Step 2 answer: high confidence, select it.\n"
                f"- If Guardian DISAGREES: check if Guardian correctly understood the question. "
                f"If Guardian misread a negation or selected an answer that contradicts what "
                f"the question asks, OVERRIDE the Guardian.\n"
                f"- Count Auditors: if 3+ Auditors independently chose a DIFFERENT answer "
                f"than Guardian, this is a strong signal to verify Guardian's logic.\n"
                f"- Guardian has authority on cultural FACTS, not on question interpretation.\n"
                f"- All option positions are equally likely to be correct.\n\n"
