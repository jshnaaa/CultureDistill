...
class BaseActorRayActor(BasePPORole):
    def init_model_from_pretrained(self, strategy: DeepspeedStrategy, pretrain):
        # instante, load ckpt, optim, scheduler, dataset
        args = strategy.args
...
class ActorModelRayActor(BaseActorRayActor):
    def fit(
        self,
...
class ActorPURETrainer(PURETrainer):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(*args, **kwargs)
        assert isinstance(vllm_engines, List)
        self.vllm_engines = vllm_engines
    ...
    def training_step(self, experience: Experience, global_steps) -> Dict[str, float]:
        return super().training_step_actor(experience)
        ...
        if self.save_hf_ckpt:
            ...
            if torch.distributed.get_rank() == 0 and os.path.exists(args.ckpt_path):
                subdirs = sorted(
                    [
                        (
                            os.path.join(args.ckpt_path, d), 
                            os.path.getmtime(os.path.join(args.ckpt_path, d))
                        )
                        for d in os.listdir(args.ckpt_path)
                        if os.path.isdir(os.path.join(args.ckpt_path, d))
                    ],
                    key=lambda x: x[1],
                )

                if len(subdirs) >= args.max_ckpt_num:
                    oldest_dir = subdirs[0][0]
                    if os.path.exists(oldest_dir):
                        shutil.rmtree(oldest_dir)
                        self.strategy.print(f"Deleted oldest ckpt {oldest_dir}")
