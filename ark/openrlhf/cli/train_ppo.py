...
def train(args):
    ...
    if not args.remote_rm_url:
        reward = get_llm_for_sequence_regression(
    ...
    if reward:
        strategy.print("mean: {}, std {}".format(reward.mean, reward.std))
    ...
    # load weights for \pi^k_{LLM}
    last_actor = torch.nn.Linear(1, 1)
    # Actor(
    #     args.pretrain,
    #     use_flash_attention_2=args.flash_attn,
    #     bf16=args.bf16,
    ...
    #     ds_config=strategy.get_ds_eval_config(offload=False),
    # )
    # load weights for reference reward
    initial_reward = torch.nn.Linear(1, 1)
    ...
    if args.gradient_checkpointing:
        ...
        if reward is not None:
            reward.gradient_checkpointing_enable(
                gradient_checkpointing_kwargs={"use_reentrant": args.gradient_checkpointing_use_reentrant}
            )
    ...
    #         reward, lr=args.actor_learning_rate, betas=args.adam_betas, weight_decay=args.l2
    #     )
    # else:
    #     reward_optim = None
    
    
    if "QMSum" in args.prompt_data or "QASPER" in args.prompt_data:
        strategy.args.input_key = "prompt"
        strategy.args.answer_key = "completions"
    ...
    # 5 models: actor, reward, initial_actor, last_actor, initial_reward
    (
        (actor, actor_optim, actor_scheduler),
        # (reward, reward_optim, reward_scheduler),
        reward,
        initial_actor,
        # last_actor,
        # initial_reward,
        ...
        # last_actor,
        # initial_reward,
        is_rlhf=True,  # handle corner case for ptx loss
    )

    # initial_actor.to('cpu')
    last_actor.to('cpu')
    initial_reward.to('cpu')
    ...
    if args.load_checkpoint and os.path.exists(os.path.join(args.ckpt_path, "_actor")):
        ckpt_path = args.ckpt_path  # "./ckpt/checkpoints_ppo"
        _, states = strategy.load_ckpt(actor.model, os.path.join(ckpt_path, "_actor"))
        consumed_samples = states["consumed_samples"]
        strategy.print(f"Loaded the checkpoint: {ckpt_path}, consumed_samples: {consumed_samples}")
    ...
    strategy.save_models(
        [ema_model if args.enable_ema else actor, reward],
        ['llm', 'reward'],
        tokenizer,
        args.save_path,
    )
...
if __name__ == "__main__":
    ...
    if "QMSum" in args.prompt_data or "QASPER" in args.prompt_data or "HotpotQA" in args.prompt_data:
        assert not args.input_template, "QMSum, QASPER, or HotpotQA should not use the default input template."
    ...
    if args.save_path is None:
        args.save_path = f"outputs/{args.reward_pretrain.split('/')[-1]}_grpo_{args.pretrain.split('/')[-1]}"
