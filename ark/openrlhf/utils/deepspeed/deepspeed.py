...
class DeepspeedStrategy(ABC):
    ...
    def __init__(
        ...
        self.distributed_training = getattr(args, "distributed_training", True)
        self.is_rlhf = False
    ...
    def setup_local(self):
        ...
        # For DeepSpeed to work, we need to initialize torch.distributed even in single-GPU mode
    ...
    def setup_ring_attn(self):
        ...
        # Ring attention requires distributed training
    ...
    def _ds_init_train_model(self, model, optim, scheduler):
        ...
        # In non-distributed mode, DeepSpeed should not try to initialize distributed
        dist_init_required = self.distributed_training
    ...
    def save_models(self, models: List[nn.Module], names: List[str], tokenizer, output_dir: str, **kwargs) -> None:
        assert len(models) == len(names) and 'llm' in names
        
        for model, name in zip(models, names):
            output_dir_ = os.path.join(output_dir, name)
            if self.is_rank_0():
            ...
            if self.is_rank_0():
                ...
                if isinstance(model_to_save, PeftModel):
                    model_to_save.save_pretrained(output_dir_, **kwargs)
                    if self.stage == 3:
                        torch.save(
                            get_peft_model_state_dict(model_to_save, output_state_dict),
                            os.path.join(output_dir_, "adapter_model.bin"),
                        )
                        filename = os.path.join(output_dir_, "adapter_model.safetensors")
                        if os.path.exists(filename):
                            os.remove(filename)
                else:
                    # save model
                    model_to_save.save_pretrained(output_dir_, state_dict=output_state_dict, **kwargs)

                # save config
                output_config_file = os.path.join(output_dir_, "config.json")
                ...
                if os.path.exists(train_from_model_path):
                    for filename in os.listdir(train_from_model_path):
                        if filename.endswith(".py"):
                            shutil.copy(os.path.join(train_from_model_path, filename), os.path.join(output_dir_, filename))
    ...
    def all_reduce(self, data, op="mean"):
        ...
        if isinstance(data, dict):
            ...
            # In non-distributed mode, just return the data as-is
            if not self.distributed_training:
    ...
    def print(self, *msg):
        if not self.distributed_training or self.is_rank_0():
            print(*msg)

    def is_rank_0(self) -> bool:
        return not self.distributed_training or dist.get_rank() == 0
